...
{
    public partial class ImportCATGVControl : System.Web.UI.UserControl
    {
        #region Variable
...
ICatologieBAL itemBAL = new CatologieBAL();
        ...
        private void PopulateControls(Step step)
        {
            switch (step)
            {
                case ImportCATGVControl.Step.Step1:
                    VisableAllControl();
                    this.PanelUploadFile.Visible = true;
                    break;
                case ImportCATGVControl.Step.Step2:
                    VisableAllControl();
                    this.PanelSaveData.Visible = true;
                    ImageButtonSave.Visible = true;
                    BindDataOnGridView();
                    break;
                case ImportCATGVControl.Step.Step3:
                    VisableAllControl();
                    this.PanelMessage.Visible = true;
                    break;
                case ImportCATGVControl.Step.Error:
                    VisableAllControl();
                    this.PanelSaveData.Visible = true;
                    BindDataOnGridView();
                    break;
            ...
            if (dataset.Tables.Count > 0)
            {
                ...
                try
                {
                    foreach (DataRow row in dataset.Tables[0].Rows)
                    {
                        ...
                        itemInRow = row["manv"].ToString().Trim().ToUpper();
                        if (itemInRow.Equals("manv"))
                        {
                            stringError += ", Mã nhân viên " + Resource.ErrorD003;
                            IsError = true;
                        }
                        else
                        ...
                        if (itemInRow == "")
                        {
                            stringError += ", Tên giảng viên" + Resource.ErrorD003;
                            IsError = true;
                        }
                        else
